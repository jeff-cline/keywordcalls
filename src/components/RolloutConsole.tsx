      ...
      <div className="card p-6">
        ...
        <div className="mt-4 rounded-xl bg-[color:var(--soft)] p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {d?.campaign && <BuyerTestCall campaignId={d.campaign.id} routingNumber={d.campaign.routingNumber} />}
          ...
          <div className="text-xs text-[color:var(--muted)] mt-1">🧪 <b>Test call</b> buys a fresh number each press (unique caller ID so Ringba doesn&apos;t block it), dials {d?.campaign?.routingNumber || "the buyer"} through your laptop — you&apos;ll hear it ring and can talk when they answer.</div>
        </div>
      </div>
      )}
                ...
                const row = (t: Target, i: number, key: string) => {
                  ...
                  return (
                    <tr key={key} className={`border-b border-[color:var(--line)] last:border-0 ${t.calledBack ? "bg-[#16a34a]/10 cursor-pointer hover:bg-[#16a34a]/20" : ""}`} onClick={t.calledBack ? () => setSel(t) : undefined}>
                      ...
                      <td className="py-2 px-4"><div className="flex items-center gap-1.5">{t.connectSec ? <span className={t.connectSec >= 120 ? "text-[color:#16a34a] font-bold" : "text-[color:var(--ink)]"}>{mmss(t.connectSec)}</span> : <span className="text-[color:var(--muted)]">—</span>}{t.recordingUrl && <button onClick={(e) => { e.stopPropagation(); playRecordingUrl(`/api/campaigns/recording-audio?phone=${encodeURIComponent(t.phone)}`); }} title="Hear this call (transfer + conversation)" className="text-[color:var(--brand2)] hover:opacity-70 text-[11px] leading-none">▶</button>}{t.recordingUrl && <a href={`/api/campaigns/recording-audio?phone=${encodeURIComponent(t.phone)}&download=1`} download onClick={(e) => e.stopPropagation()} title={`Download recording as ${t.phone.replace(/\D/g, "")}.mp3`} className="text-[color:var(--muted)] hover:text-[color:var(--brand2)] text-[11px] leading-none">⬇</a>}</div></td>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setSel(null)}>
          <div className="card max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="pt-2 border-t border-[color:var(--line)] mt-2"><b>Call recording:</b> {sel.recordingUrl ? <><audio controls preload="none" src={`/api/campaigns/recording-audio?phone=${encodeURIComponent(sel.phone)}`} className="w-full h-9 mt-1" /><a href={`/api/campaigns/recording-audio?phone=${encodeURIComponent(sel.phone)}&download=1`} download className="text-[color:var(--brand2)] text-sm underline mt-1 inline-block">⬇ Download as {sel.phone.replace(/\D/g, "")}.mp3</a></> : <span className="text-[color:var(--muted)]"> not ready yet (appears a few seconds after the call)</span>}</div>
            </div>
          </div>
        </div>
      )}
